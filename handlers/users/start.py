...
async def bot_start(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        msg = f"<a href='tg://user?id={user_id}'>{full_name}</a> bazaga qo'shildi. User id: [<code>{user_id}</code>]\nBazada {count} ta foydalanuvchi bor."
        await bot.send_message(chat_id=ADMINS[0], text=msg)
    except asyncpg.exceptions.UniqueViolationError:
        # user = await db.select_user(telegram_id=message.from_user.id)
        await bot.send_message(chat_id=ADMINS[0], text=f"<a href='tg://user?id={user_id}'>{full_name}</a> bazaga oldin qo'shilgan. User id: [<code>{user_id}</code>]")
